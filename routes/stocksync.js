      ...
      try { // outer try
        ...
        for (const [handle, variantsForProduct] of byHandle) {
          ...
          if (cancelRequested) {
            ...
            send({
              type: 'done', success: true, cancelled: true, dryRun,
          ...
          try {
            ...
            if (productStatus && productStatus !== 'active') {
              ...
              log.push({
                sku: variantsForProduct[0].variantSku, handle, status: 'skipped',
            ...
            if (!dryRun && prodMfToWrite.length > 0) {
              ...
              if (mfErrors.length) {
                log.push({
                  sku: repSku, handle, status: 'warning',
            ...
            for (const shopNode of shopifyNodes) {
              ...
              // - Variant not in variant feed   → look up this variant's OWN product-level SKU
              //   in the product feed (handles products where multiple CFS product SKUs share
              //   the same Shopify product handle, e.g. UD-1243174 and UD-1243180 both on
              //   dylan-electric-recliner-sofa-3-seater-sandstone-fabric).
              ...
              if (varStock) {
                ...
                // Per-variant product-feed lookup using this variant's own SKU
                const vSkuParts = varSku.split('-');
                const vProductLevelSku = vSkuParts.length >= 2 ? `${vSkuParts[0]}-${vSkuParts[1]}` : null;
                const vProdId = vSkuParts[1] || null;
                const vProdStock = prodStockBySku.get(varSku)
                  || (vProductLevelSku ? prodStockBySku.get(vProductLevelSku) : null)
                  || (vProdId ? prodStockByProdId.get(vProdId) : null)
                  || prodStock; // final fallback to the product-representative lookup
                vDeliveryTime = vProdStock?.deliveryTime ?? deliveryTime;
                vIsShortLead = deliveryTimeIsShort(vDeliveryTime);
                vInOutStock = vIsShortLead ? 'IN STOCK' : 'OUT OF STOCK';
              }
              ...
              if (!dryRun) {
                if (varMfToWrite.length > 0) {
                  ...
                  if (vmfErrors.length) {
                    log.push({
                      sku: varSku, handle, status: 'warning',
        ...
        send({
          type: 'done', success: false, error: `Server error: ${outerErr.message}`,
